...
object NcmApiClient {
    ...
    suspend fun addToPlaylist(playlistId: Long, songId: Long): Boolean {
        return try {
            val jsonStr = executeGet("/playlist/tracks?op=add&pid=$playlistId&tracks=$songId")
            val json = JSONObject(jsonStr)
            val code = json.optInt("code", json.optInt("status", 0))
            code == 200 || code == 502
        } catch (e: Exception) {
            e.printStackTrace()
            false
    ...
    // 18. 获取歌曲音乐百科
    suspend fun getSongWiki(songId: Long): SongWiki? {
        return try {
            val jsonStr = executeGet("/song/wiki/summary?id=$songId")
            val json = JSONObject(jsonStr)
